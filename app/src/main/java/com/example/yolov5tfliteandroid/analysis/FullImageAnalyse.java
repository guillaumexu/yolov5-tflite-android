...
public class FullImageAnalyse implements ImageAnalysis.Analyzer {
    ...
    ImageView        mBoxLabelCanvas;
    PreviewView      mPreviewView;
    int              mRotation;
    private TextView mInferenceTimeTextView;
    private TextView mFrameSizeTextView;
    ...
    public FullImageAnalyse(Context context,
        ...
        this.mBoxLabelCanvas = boxLabelCanvas;
        this.mPreviewView = previewView;
        this.mInferenceTimeTextView = inferenceTimeTextView;
        this.mFrameSizeTextView = frameSizeTextView;
//        this.mImageProcess = new ImageProcess();
        this.mYolov5TFLiteDetector = yolov5TFLiteDetector;
    }

    @Override
    public void analyze(@NonNull ImageProxy image) {
        int previewHeight = this.mPreviewView.getHeight();
        int previewWidth = this.mPreviewView.getWidth();

        // 这里Observable将image analyse的逻辑放到子线程计算, 渲染UI的时候再拿回来对应的数据, 避免前端UI卡顿
        Observable.create((ObservableEmitter<Result> emitter) -> {
                    long start = System.currentTimeMillis(); //开始时间

                    byte[][] yuvBytes = new byte[3][]; //创建三个元素的byte数组
                    ImageProxy.PlaneProxy[] planes = image.getPlanes(); //得到image的平面
                    int imageHeight = image.getHeight();                //得到image的高
                    int imagewWidth = image.getWidth();                 //得到image的宽

                    mImageProcess.fillBytes(planes, yuvBytes);
                    int yRowStride = planes[0].getRowStride();
                    final int uvRowStride = planes[1].getRowStride();
                    final int uvPixelStride = planes[1].getPixelStride();

                    int[] rgbBytes = new int[imageHeight * imagewWidth];
                    mImageProcess.YUV420ToARGB8888(
                            yuvBytes[0],
                            yuvBytes[1],
                            yuvBytes[2],
                            imagewWidth,
                    ...
                    Log.e("image", "previewHeight" + previewHeight + "previewWidth" + previewWidth);
                    Matrix fullScreenTransform = mImageProcess.getTransformationMatrix(
                    ...
                    Matrix previewToModelTransform =
                            mImageProcess.getTransformationMatrix(
...
//            emitter.onNext(new Result(costTime, imageBitmap));
                ...
                .subscribe((Result result) -> {
                    mBoxLabelCanvas.setImageBitmap(result.bitmap);
                    mFrameSizeTextView.setText(previewHeight + "x" + previewWidth);
                    mInferenceTimeTextView.setText(Long.toString(result.costTime) + "ms");
                });
    }
